...
const Todo = ({ dragHandleProps }) => {
  ...
  return (
    <div className="figma-glass-static todo-widget rounded-[26px] px-4 py-3 text-white font-gilroy-medium w-full h-full select-none flex flex-col justify-between shadow-2xl relative overflow-hidden">
      {/* Header Row */}
      ...
      <div className="w-full flex-1 min-h-0 overflow-y-auto scrollbar-hide flex flex-col gap-1.5 z-10 pr-0.5">
        {tasks.length === 0 && (
          <p className="winter-todo-empty" aria-hidden="true">DO THE NEXT THING.</p>
        )}
        {tasks.map((task) => (
          <div
            key={task.id}
