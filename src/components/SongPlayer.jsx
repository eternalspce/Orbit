...
const SongPlayer = ({ dragHandleProps, playlistUrl, autoPlay, customVideo, stations }) => {
  ...
  // Initialize & update audio element src whenever stream URL or play state changes
  useEffect(() => {
    if (!audioRef.current) return;
    const currentAudio = audioRef.current;
    
    // Set src and reload audio buffer if changed
    if (currentAudio.src !== activeStreamUrl) {
